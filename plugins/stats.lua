...
local function run(msg, matches)
  if matches[1]:lower() == 'Dokhaniat' then -- Put everything you like :)
    local about = _config.about_text
    local name = user_print_name(msg.from)
    savelog(msg.to.id, name.." ["..msg.from.id.."] used /Dokhaniat ")
    return about
  end 
  ...
  if matches[1]:lower() == "آمار" then
    ...
    if matches[2] == "Dokhaniat" then -- Put everything you like :)
...
return {
  patterns = {
    ...
    "^(لیست آمار)$",
    "^(امار) (گروه) (%d+)",
    "^(امار) (Dokhaniat)",-- Put everything you like :)
		"^[!/@#$%^&*()_+](Dokhaniat)"-- Put everything you like :)
    }, 
  run = run
}
